...
<div class="mainDiv">
	<div class= "left">
		...
		<fmt:formatNumber minFractionDigits="2" maxFractionDigits="2" value="${auction.buyOutPrice}"/><br>
	...
	<div class="upperRight">
		...
		<c:choose>
		    <c:when test="${empty auction.auctionBids}">
		       <fmt:formatNumber minFractionDigits="2" maxFractionDigits="2" value="${auction.startPrice}"/><br>
		    </c:when>
		    <c:otherwise>
		        <fmt:formatNumber minFractionDigits="2" maxFractionDigits="2" value="${auction.auctionBids[0].bidValue}"/><br>
		    </c:otherwise>
		</c:choose>
				...
				<table class="dataTable">
					...
					<tbody >
						<c:forEach items="${auction.auctionBids}" var="auctionBid">
							<tr>
								<td><fmt:formatDate type="both" dateStyle="medium" timeStyle="short" value="${auctionBid.bidDate}"/></td>
								<td><c:out value="${auctionBid.user.username}"/></td>
								<td><fmt:formatNumber minFractionDigits="2" maxFractionDigits="2" value="${auctionBid.bidValue}"/></td>
							</tr>
						</c:forEach>
					</tbody>
					</table>
